...
import 'package:flutter/src/material/colors.dart' as cc;
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        ARScenarioScreen(liquid_test: selectedDisaster),
                  ),
                );
              },
...
class ARScenarioScreen extends StatefulWidget {
  final String liquid_test;

  const ARScenarioScreen({Key? key, required this.liquid_test}) : super(key: key);

  @override
  _ARScenarioScreenState createState() => _ARScenarioScreenState();
}

class _ARScenarioScreenState extends State<ARScenarioScreen> {
  late ARViewController arViewController;

  @override
  void initState() {
    super.initState();
    // Request camera permissions here if needed (Platform-specific code)
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('AR View: ${widget.liquid_test}'),
      ),
      body: ARView(
      void  onARViewCreated: (ARViewController controller) {
    arViewController = controller;

    // Enable AR features like the camera feed
    arViewController.startTracking();

    // Add the 3D model to the AR scene
    arViewController.addNode(
    ARNode(
    type: NodeType.localGLTF2,
    uri: "assets/models/liquid_test.glb",
  ...
  @override
  void dispose() {
    arViewController.stopTracking(); // Stop AR tracking when leaving the screen
    super.dispose();
  }
}
      ...
      body: Center(
        child: Column(
          ...
          children: [
            const Text(
              'Game Over',
              style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: cc.Colors.blue),
            ),
            const SizedBox(height: 20),
            Text(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            const Text(
              'Congratulations!',
              style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: cc.Colors.green),
            ),
            const SizedBox(height: 20),
            Text(
              'Your final score: $score',
